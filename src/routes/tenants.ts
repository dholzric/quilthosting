...
export function validateDesignSettings(
  ...
  if (out.site !== undefined) {
    ...
    } else if (out.site.renderer !== undefined && out.site.renderer !== "sections") {
      // There is one renderer since v0.61.0; "legacy" is no longer a value a
      // tenant can be put back onto.
      issues.push({ path: "settings.site.renderer", message: 'renderer must be "sections"' });
    }
  }
