...
**Tenant websites are server-rendered by one renderer** (`src/routes/site.ts` `serveSite`, `src/lib/site/render.ts`): sections (`src/lib/site/sections/`, schema frozen; legacy blocks normalize 1:1) styled by a design-token model (`src/lib/site/design/`: 42 palettes in 8 families, 4 page tones, 12 type pairs, derived contrast-checked roles, `buildDesignVars` → `--qh-*` variables consumed by `public/qh-site.css`; interactive islands in `public/qh-site.js`). There is exactly one renderer: the classic `public/guild.html` shell, its `settings.site.renderer === "legacy"` flag (migration 0026), the "Try the new design" / "Back to classic" switch and `/g/:slug/__preview` were all removed in v0.61.0 once every tenant had been converted. `settings.site.renderer` now accepts only `"sections"`. New guilds are seeded from a kit (`src/lib/site/kits/*.json`, Heritage by default; `QuiltHostingTemplates.md` is the authoring brief, `npm run kits:validate` the gate).

**Website builder = draft → preview → publish** (`src/routes/pages.ts`, `src/lib/pageDrafts.ts`, migration 0025): `PUT /pages/:id/draft` autosaves into `draft_*` columns, `POST /pages/:id/publish` promotes the draft in one batch and snapshots the previous live content into `page_revisions`; every write is CAS-guarded on `pages.revision` (409 on conflict). `DELETE` soft-deletes (`deleted_at`), slug changes write `page_redirects`. The shared editor lives in `public/admin.html` (`qhOpenPageEditor`) for both guild and business tenants, and both preview through the SSR renderer.
