...
| `unparseable_date` | yes | Some renewal/expiry dates could not be read and will be left blank | A row's end/expiry/renewal/expiration value doesn't parse as a date. On a real import, where the row also names a level, the row falls through to exactly the same chosen-date path as a row with no expiry column at all, and the row gets a per-row `level_without_end_date` **error** naming the date that was chosen. The aggregate `level_without_end_date` **warning** does not also fire for it — `buildWarnings()` collects these rows into `badDates`, and its `levelWithoutEnd` bucket is the `else` branch of the same `if (endRaw)`, so the two aggregates partition the rows rather than double-counting them. That is deliberate: these rows are already represented on screen by this warning, which is **lossy** (the guild believes they supplied a date and did not get it — unlike `level_without_end_date`, where they supplied none) and therefore the stronger signal, forcing `partial`. Reporting the same rows under a second, informational code would inflate two counts that an admin reasonably reads as disjoint.
...
| `level_without_end_date` | **no** | Some rows name a membership level but no renewal/expiry date. We will set each of those renewal dates to one full term of the level's duration counted from today… | As an aggregate **warning**, fires when a row resolves a level and the file supplies **no** end date — no expiry column at all, or an empty cell (the commonest Wild Apricot roster shape). A cell that is present but *unreadable* takes the same chosen-date path in the code, and gets the same per-row **error**, but is aggregated under `unparseable_date` instead (see that row) so the two counts stay disjoint. `computeMembershipEnd()` (`src/lib/memberships.ts`) then picks the level's `duration_months` counted from `max(startDate, now)` — today, unless the row's start date is in the future, in which case a deliberately post-dated term keeps its full length. **It never shortens an existing membership:** before writing, the import compares against the member's current active membership and keeps that one's `end_date` when it is later, or `NULL` (open-ended). Replacing an *earlier* end date is a renewal, and the per-row reason names both dates. When the level and the resolved end date both match what the member already holds, the import writes **nothing at all** — no expire, no insert — so re-running a roster neither churns the membership ledger nor disturbs the row. When something did change, `auto_renew` and `stripe_subscription_id` are carried onto the new membership (the billing relationship belongs to the member, not to one term); `amount_paid_cents` is carried only when the term is unchanged, since copying it onto a new term would record a payment that never happened. An explicit, parseable date in the file always wins outright — with one refinement: a date naming the same calendar day as the stored `end_date` keeps the stored instant, because `export.csv` truncates to `YYYY-MM-DD` and a plain round trip would otherwise re-parse as UTC midnight. Deliberately **not** lossy: nothing in the file is discarded (there was no renewal information to discard) and nothing in the database is either. Marking it lossy would make `partial` fire on nearly every first migration, the same reasoning that excluded `joined_at_ignored_on_update`. The aggregate warning counts every row the scan matched; the per-row `errors` entries are pushed only for rows whose membership was actually created, so plan-limited, membership-failed and skipped rows are absent from the per-row list (they carry their own kind instead), and a dry run has no `errors` array at all.
...
`lossy_warning_codes` is the serialized `LOSSY_WARNING_CODES` set, returned
by the import route and by the per-batch errors route. Clients must use it to
tell an informational warning from a lossy one rather than approximating with
`warnings.length === 0` — `level_without_end_date` fires on every
Level-column file, so that approximation misclassifies the commonest roster
shape (it made a plan-capped free guild see "Import finished with problems"
for a file with nothing wrong with it).
