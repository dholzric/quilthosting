...
| `level_without_end_date` | **no** | Some rows name a membership level but no renewal/expiry date. We will set each of those renewal dates to one full term of the level's duration counted from today… | Fires when a row resolves a level and the file supplies **no usable** end date — either no expiry column at all (the commonest Wild Apricot roster shape) or a value that doesn't parse (`unparseable_date` also fires for that row). `computeMembershipEnd()` (`src/lib/memberships.ts`) then picks the level's `duration_months` counted from `max(startDate, now)` — today, unless the row's start date is in the future, in which case a deliberately post-dated term keeps its full length. **It never shortens an existing membership:** before writing, the import compares against the member's current active membership and keeps that one's `end_date` when it is later, or `NULL` (open-ended). Replacing an *earlier* end date is a renewal, and the per-row reason names both dates. An explicit, parseable date in the file always wins outright — with one refinement: a date naming the same calendar day as the stored `end_date` keeps the stored instant, because `export.csv` truncates to `YYYY-MM-DD` and a plain round trip would otherwise re-parse as UTC midnight. Deliberately **not** lossy: nothing in the file is discarded (there was no renewal information to discard) and nothing in the database is either. Marking it lossy would make `partial` fire on nearly every first migration, the same reasoning that excluded `joined_at_ignored_on_update`. The aggregate warning counts every row the scan matched; the per-row `errors` entries are pushed only for rows whose membership was actually created, so plan-limited, membership-failed and skipped rows are absent from the per-row list (they carry their own kind instead), and a dry run has no `errors` array at all.
